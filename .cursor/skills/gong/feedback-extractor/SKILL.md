...
Use this skill after `/gong-call-selector` to produce `data/feedback.json`.
...
- Env vars in `.env` and `.env.local`

## Instructions

1. From repo root, run:
   - `bash .cursor/skills/gong/feedback-extractor/scripts/extract-feedback.sh`
2. For MVP:
   - `CALL_LIMIT=10 USE_PROCESSED_CALLS=false bash .cursor/skills/gong/feedback-extractor/scripts/extract-feedback.sh`
3. Confirm output:
   - `data/feedback.json`
   - optional `data/processed-calls.json`

## Output Contract

- Primary file: `data/feedback.json`
- Contract docs:
  - `contracts/feedback.md`
  - `contracts/extraction-rubric.md`
  - `.cursor/skills/gong/feedback-extractor/references/feedback-schema.md`
...
- **Exclusions:** Do not extract non-Cursor issues (e.g. Zoom notifications), meeting-only logistics (e.g. “share the slides”), or generic session praise with no product tie — see `contracts/extraction-rubric.md` § Exclusions.
